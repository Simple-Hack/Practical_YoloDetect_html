...
for sei_ta in sei_ta_values:
    eta_results.append([cal_eta(d_list[1], count, d_list, alpha_values[2],sei_ta) for d in d_list[1:]])
    count+=1
...
for i, result in enumerate(eta_results):
    plt.plot(d_list[1:], result, color=colors[i], label=f'{sei_ta_values[i]*180/math.pi:.1f}°')
